...
Поздний выезд (после 12:00) — по возможности и за доплату, решает администратор Асель. Если спрашивают: «Поздний выезд по возможности и за доплату ||| Позвоните нашему администратору Асель: +7-913-693-68-19 ||| Если не дозвонитесь, напишите здесь». Пиши именно «поздний выезд», без других слов.
...
def maybe_notify_handoff(chat_id, chat_type, guest_name, guest_msg, reply):
    """Если бот направил гостя к Асели (в ответе её телефон) — пингуем её в Telegram, чтобы зашла в чат.
    Дедуп: не чаще одного уведомления в 10 минут на один чат."""
    ...
    notify_asel(
        "🔔 Гость ждёт вашего ответа\n"
        f"Имя: {guest_name or 'не указано'}\n"
        f"Мессенджер: {chat_type}\n"
        f"Чат/телефон: {chat_id}\n"
        f"Спросил: {(guest_msg or '')[:200]}\n"
        "Зайдите в этот чат в приложении и ответьте сами."
    )
...
def flush_chat(chat_id):
    ...
    try:
        ...
        # Бот направил гостя к Асели? Пингуем её в Telegram, чтобы зашла в чат
        guest_name = next((m.get("contact", {}).get("name") for m in msgs
                           if m.get("contact", {}).get("name")), None)
        maybe_notify_handoff(chat_id, chat_type, guest_name, combined, ai_reply)
    except Exception as e:
