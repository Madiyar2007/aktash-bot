...
def build_prefilled_link(date_from, date_to, rooms, phone=None):
    ...
    for mid, g in rooms:
        g = max(1, int(g))
        total += g
        suffix = "" if g <= 2 else f"_0.1.{g - 2}"   # доп. места кодируются хвостом
        key = f"{mid}{suffix}"
        if key in groups:
            groups[key]["c"] += 1
        else:
            groups[key] = {"c": 1, "bv": (g if g > 2 else 0)}
...
def get_ai_response(user_message, chat_id):
    ...
    # Модель может указать выбранные номера: [BOOKING_LINK rooms=Лофт:4, Номер Стандарт:2] — тогда
    # собираем ссылку на оформление с УЖЕ выбранными номерами. Иначе — обычная ссылка (гость выберет сам).
    ...
    if tok:
        ...
        if df and dt and in_season(datetime.strptime(df, '%Y-%m-%d')):
            ...
            if not link:  # номера не заданы или не распознаны — обычная ссылка с датами и гостями
